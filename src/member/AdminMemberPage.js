...
const MemberList = () => {
    ...
    useEffect(() => {
        ...
        const fetchMembers = async (page) => {
            try {
                ...
                const response = await axios.get(`http://ec2-3-34-78-114.ap-northeast-2.compute.amazonaws.com:8080/api/admin/member?page=${page}`, {
                    headers: {
    ...
    const handleDeleteMember = async (memberId) => {
        try {
            ...
            const token = localStorage.getItem('token'); // 토큰 가져오기
            await axios.delete(`http://ec2-3-34-78-114.ap-northeast-2.compute.amazonaws.com:8080/api/admin/member/${memberId}`, {
                headers: {
                    Authorization: `Bearer ${token}`, // 토큰을 Authorization 헤더에 포함
                },
            });
