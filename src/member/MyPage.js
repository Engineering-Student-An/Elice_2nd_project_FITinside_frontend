...
const MyPage = () => {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            try {
                ...
                const response = await axios.get('/fr/api/user/me', {
                    headers: {
    ...
    const handleSaveName = async () => {
        try {
            ...
            await axios.put(
                '/fr/api/user/username', // 이름 수정 API 엔드포인트
                { userName: newName },
    ...
    const handleSavePhone = async () => {
        try {
            ...
            const token = localStorage.getItem('token');
            await axios.put(
                '/fr/api/user/phone', // 전화번호 수정 API 엔드포인트
                {
    ...
    const handleDeleteAccount = async () => {
        try {
            const token = localStorage.getItem('token');
            await axios.delete('/fr/api/user/delete', {
                headers: { Authorization: `Bearer ${token}` }
            });
