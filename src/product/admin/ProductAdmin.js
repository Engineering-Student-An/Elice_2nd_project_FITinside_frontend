...
const ProductAdmin = () => {
    ...
    const fetchProducts = async (pageNumber) => {
        const endpoint = searchType === 'productName' ? '/fr/api/products' : '/fr/api/products/byCategory'; // 검색 타입에 따라 다른 엔드포인트 호출
    ...
    const handleDeleteClick = async (productId) => {
        ...
        try {
            ...
            const response = await axios.delete(`/fr/api/admin/products/${productId}`, {
                headers: {
                    'Authorization': `Bearer ${token}`,  // Authorization 헤더 추가
                },
            });
            ...
            if (error.response && error.response.status === 401) {
                ...
                try {
                    ...
                    // 토큰 갱신 후 다시 삭제 요청
                    const newResponse = await axios.delete(`/fr/api/admin/products/${productId}`, {
                        headers: {
                            'Authorization': `Bearer ${localStorage.getItem('token')}`,  // 새로 발급된 토큰 사용
                        },
                    });
