...
const ProductList = () => {
    ...
    const fetchCategoryName = async () => {
        try {
            const response = await axios.get(`http://ec2-3-34-78-114.ap-northeast-2.compute.amazonaws.com:8080/api/categories/${categoryId}`); // 카테고리 정보를 가져오는 API 호출
    ...
    const fetchProducts = async () => {
        try {
            const response = await axios.get(`http://ec2-3-34-78-114.ap-northeast-2.compute.amazonaws.com:8080/api/products/category/${categoryId}`, {
                params: {
                    page,
                    size,
                    sortField,
