...
const ProductList = () => {
    ...
    const fetchCategoryName = async () => {
        try {
            const response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_PROXY}/api/categories/${categoryId}`); // 카테고리 정보를 가져오는 API 호출
    ...
    const fetchProducts = async () => {
        try {
            const response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_PROXY}/api/products/category/${categoryId}`, {
                params: {
                    page,
                    size,
                    sortField,
