...
const CouponList = () => {
    ...
    const fetchCoupons = async (page, includeInActiveCoupons) => {
        try {
            const response = await axios.get(`http://ec2-3-34-78-114.ap-northeast-2.compute.amazonaws.com:8080/api/coupons`, {
                params: {
                    page: page,
    ...
    const handleOrderHistoryClick = async (couponId) => {
        try {
            const response = await axios.get(`http://ec2-3-34-78-114.ap-northeast-2.compute.amazonaws.com:8080/api/coupons/${couponId}/order`, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
            });
