...
const CouponList = () => {
    ...
    const fetchWelcomeCoupons = async () => {
        try {
            // 첫 번째 요청: 웰컴 쿠폰 가져오기
            const response = await axios.get('/fr/api/coupons/welcome', {
                headers: {
            ...
            const myWelcomeResponse = await axios.get('/fr/api/coupons/my-welcome', {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                },
            });
    ...
    const handleCouponSubmit = async (code) => {
        try {
            const response = await axios.post('/fr/api/coupons', code, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`,
