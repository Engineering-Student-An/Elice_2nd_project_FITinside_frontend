...
const CouponList = () => {
    ...
    const fetchWelcomeCoupons = async () => {
        try {
            // 첫 번째 요청: 웰컴 쿠폰 가져오기
            const response = await axios.get('http://ec2-3-34-78-114.ap-northeast-2.compute.amazonaws.com:8080/api/coupons/welcome', {
                headers: {
            ...
            const myWelcomeResponse = await axios.get('http://ec2-3-34-78-114.ap-northeast-2.compute.amazonaws.com:8080/api/coupons/my-welcome', {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                },
            });
    ...
    const handleCouponSubmit = async (code) => {
        try {
            const response = await axios.post('http://ec2-3-34-78-114.ap-northeast-2.compute.amazonaws.com:8080/api/coupons', code, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`,
