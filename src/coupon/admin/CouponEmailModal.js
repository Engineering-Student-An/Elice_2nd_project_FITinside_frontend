...
const CouponEmailModal = ({ isOpen, onRequestClose, coupon }) => {
    ...
    const fetchMembers = async (id) => {
        ...
        setError(null);
        try {
            const response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_PROXY}/api/admin/coupons/${id}/members`, {
                headers: {
    ...
    const sendCouponEmails = async () => {
        ...
        try {
            await Promise.all(selectedEmails.map(async (email, index) => {
                ...
                // 실제 이메일 전송 요청
                const response = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_PROXY}/api/admin/coupons/email`, {
                    couponId: coupon.id,
                    address: email,
