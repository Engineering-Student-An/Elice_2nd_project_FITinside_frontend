...
const AddressList = () => {
    ...
    const handleSave = async () => {
        ...
        if (isAdding) {
            // 배송지 추가 로직
            try {
                const response = await axios.post('/fr/api/addresses', dataToSend, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
            ...
            // 배송지 수정 로직
            try {
                const response = await axios.patch(`/fr/api/addresses/${selectedAddress.addressId}`, dataToSend, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
    ...
    const handleDelete = async (addressId) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`/fr/api/addresses/${addressId}`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
    ...
    const handleDefaultChange = async (addressId, isChecked) => {
        try {
            const token = localStorage.getItem('token');
            await axios.patch(`/fr/api/addresses/${addressId}/default`, null, {
                headers: {
                    'Authorization': `Bearer ${token}`
                },
                params: {
