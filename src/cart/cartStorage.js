...
export const fetchProduct = async (id) => {
    try {
        const response = await axios.get(`http://ec2-3-34-78-114.ap-northeast-2.compute.amazonaws.com:8080/api/products/${id}`, {
            headers: {
        ...
        try {
            ...
            // 토큰 갱신 후 다시 요청
            const newResponse = await axios.get(`http://ec2-3-34-78-114.ap-northeast-2.compute.amazonaws.com:8080/api/products/${id}`, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
            });
...
export const fetchAndMergeCartData = async (token) => {
    try {
        const response = await axios.get('http://ec2-3-34-78-114.ap-northeast-2.compute.amazonaws.com:8080/api/carts', {
            headers: {
        ...
        try {
            ...
            // 토큰 갱신 후 다시 요청
            const newResponse = await axios.get('http://ec2-3-34-78-114.ap-northeast-2.compute.amazonaws.com:8080/api/carts', {
                headers: {
...
const updateDBWithDifferences = async (localCart) => {
    ...
    for (const item of localCart) {
        ...
        if (!dbItem) {
            ...
            try {
                await axios.post('http://ec2-3-34-78-114.ap-northeast-2.compute.amazonaws.com:8080/api/carts', requestData, {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`,
...
const updateCartItem = async (item) => {
    ...
    try {
        await axios.put('http://ec2-3-34-78-114.ap-northeast-2.compute.amazonaws.com:8080/api/carts', requestData, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`,
                'Content-Type': 'application/json',
            },
...
const deleteCartItem = async (item) => {
    ...
    if(localStorage.getItem('token') === null) return;
    try {
        await axios.delete(`http://ec2-3-34-78-114.ap-northeast-2.compute.amazonaws.com:8080/api/carts/${item.id}`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`,
                'Content-Type': 'application/json',
            },
