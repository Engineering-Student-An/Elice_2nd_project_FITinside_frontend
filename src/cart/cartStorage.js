...
export const fetchProduct = async (id) => {
    try {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_PROXY}/api/products/${id}`, {
            headers: {
        ...
        try {
            ...
            // 토큰 갱신 후 다시 요청
            const newResponse = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_PROXY}/api/products/${id}`, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
            });
...
export const fetchAndMergeCartData = async (token) => {
    try {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_PROXY}/api/carts`, {
            headers: {
        ...
        try {
            ...
            // 토큰 갱신 후 다시 요청
            const newResponse = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_PROXY}/api/carts`, {
                headers: {
...
const updateDBWithDifferences = async (localCart) => {
    ...
    for (const item of localCart) {
        ...
        if (!dbItem) {
            ...
            try {
                await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_PROXY}/api/carts`, requestData, {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`,
...
const updateCartItem = async (item) => {
    ...
    try {
        await axios.put(`${process.env.NEXT_PUBLIC_BACKEND_PROXY}/api/carts`, requestData, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`,
                'Content-Type': 'application/json',
            },
...
const deleteCartItem = async (item) => {
    ...
    if(localStorage.getItem('token') === null) return;
    try {
        await axios.delete(`${process.env.NEXT_PUBLIC_BACKEND_PROXY}/api/carts/${item.id}`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`,
                'Content-Type': 'application/json',
            },
