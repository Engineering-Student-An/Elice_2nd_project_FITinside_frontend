...
export const fetchProduct = async (id) => {
    try {
        ...
        try {
            ...
            // 토큰 갱신 후 다시 요청
            const newResponse = await axios.get(`https://zaswdsrcjxykrnsf.tunnel-pt.elice.io/api/products/${id}`, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
            });
...
export const fetchAndMergeCartData = async (token) => {
    try {
        const response = await axios.get('https://zaswdsrcjxykrnsf.tunnel-pt.elice.io/api/carts', {
            headers: {
        ...
        try {
            ...
            // 토큰 갱신 후 다시 요청
            const newResponse = await axios.get('https://zaswdsrcjxykrnsf.tunnel-pt.elice.io/api/carts', {
                headers: {
...
const updateDBWithDifferences = async (localCart) => {
    ...
    for (const item of localCart) {
        ...
        if (!dbItem) {
            ...
            try {
                await axios.post('https://zaswdsrcjxykrnsf.tunnel-pt.elice.io/api/carts', requestData, {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`,
...
const updateCartItem = async (item) => {
    ...
    try {
        await axios.put('https://zaswdsrcjxykrnsf.tunnel-pt.elice.io/api/carts', requestData, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`,
                'Content-Type': 'application/json',
            },
...
const deleteCartItem = async (item) => {
    ...
    if(localStorage.getItem('token') === null) return;
    try {
        await axios.delete(`https://zaswdsrcjxykrnsf.tunnel-pt.elice.io/api/carts/${item.id}`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`,
                'Content-Type': 'application/json',
            },
